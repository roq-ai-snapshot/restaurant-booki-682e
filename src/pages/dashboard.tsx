...
import { WaiterListPage as ShowCaseList } from 'pages/waiters';

function DashboardPage() {
  const { session } = useSession();

  const waiterStat = useSWR<PaginatedInterface<WaiterInterface>>(
    () => `/waiters`,
    () => getWaiters({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Waiters',
      result: waiterStat?.data?.totalCount,
      isLoading: waiterStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`waiters`} passHref legacyBehavior>
                  <Button
