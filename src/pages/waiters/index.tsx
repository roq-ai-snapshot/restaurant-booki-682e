...
type ColumnType = ColumnDef<WaiterInterface, unknown>;
...
export function WaiterListPage(props: WaiterListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<WaiterInterface>>(
    () => `/waiters?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: WaiterInterface) => {
    if (hasAccess('waiter', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/waiters/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.shift_start ? format(parseISO(record?.shift_start as unknown as string), 'dd-MM-yyyy') : '',
    },
    {
      id: 'shift_end',
      header: 'Shift End',
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.shift_end ? format(parseISO(record?.shift_end as unknown as string), 'dd-MM-yyyy') : '',
    },
    { id: 'uniform_number', header: 'uniform_number', accessorKey: 'uniform_number' },
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="waiter" />
        </Flex>

        {hasAccess('waiter', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/waiters/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
