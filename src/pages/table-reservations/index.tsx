...
type ColumnType = ColumnDef<TableReservationInterface, unknown>;

interface TableReservationListPageProps {
...
export function TableReservationListPage(props: TableReservationListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TableReservationInterface>>(
    () => `/table-reservations?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: TableReservationInterface) => {
    if (hasAccess('table_reservation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/table-reservations/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'number_of_guests', header: 'number_of_guests', accessorKey: 'number_of_guests' },
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="table_reservation" />
        </Flex>

        {hasAccess('table_reservation', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/table-reservations/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
